        ...
        >
          ...
          <div v-else-if="step == 2">
            <div class="nickname__box">
              ...
              <LoginBtn color="#fff" :background="btnBg" @btnClick="step3"
                >完成</LoginBtn
              >
            </div>
          </div>
  ...
  methods: {
    ...
    stepTwo(val) {
      verifyAPI(this.regForm.phone, val, this.regForm.countrycode)
        .then(async () => {
          const { data: result } = await existenceAPI(
            this.regForm.phone,
            this.regForm.countrycode
          )
          if (result.hasPassword === true) {
            this.mode('login')
            return this.$message.error('此手机号已被注册')
          }
          this.step = 2
    ...
    // 第三步
    async step3() {
      this.$refs.regFormRef.validateField('nickname', async (validate) => {
        if (validate) return
